...
[abstract]
You can use the xref:concurrent-mutations-cluster.adoc[CAS] value to control how concurrent document modifications are handled.
...
[source,python]
----
def replace(self, docid:str, new_value:Any, oldCas:int):
        existing = self.kvStore.get(docid)
    if not existing:
        raise couchbase.exceptions.NotFoundError()
    elif oldCas != 0 and oldCas != existing.cas:
        raise couchbase.exceptions.KeyExistsError()
    existing.cas+=1
    new_cas = existing.cas
...
The following blocks show how to use [.api]`lock` and [.api]`unlock` with the Python SDK.
This process should be similar for other SDKs.

[source,python]
----
result=cb.lock("key",5)
if unlock_only:
    cb.unlock("key",result.cas)
else:
    cb.replace("key", "value", cas=result.cas)
----

The handler will unlock the item either via an explicit unlock operation ([.api]`couchbase.bucket.Bucket.unlock`) or implicitly via modifying the item with the correct CAS.

If the item has already been locked, the server will respond with [.var]`couchbase.exceptions.TemporaryFailError` which means that the operation could not be executed temporarily, but may succeed later on.
