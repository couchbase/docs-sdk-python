...
From async and reactive APIs, to unit tests and handling errors.
...
Note, a slight performance improvement can be found by <<#rto-vs-performance,turning off observability>>,
...
====
In version 4.6.0 of the {name-sdk},
fully featured Observability (switched on by default) replaced the limited information from the underlying {cpp} core.
This slighty reduced overall performance of the SDK,

If absolute performance is critical to your application, then observability features can be turned off
(see `ClusterOptions` in the https://docs.couchbase.com/sdk-api/couchbase-python-client/couchbase_api/options.html#clusteroptions[API reference]).
But be sure that your performance need is strong --
observability information is invaluable in diagnosing network bottlenecks and problems.
====
