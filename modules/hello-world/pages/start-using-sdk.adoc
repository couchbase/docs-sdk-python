...
Please note the 'pip' comnmands below will install the latest prerelease version.
If you wish to install a specific version, use 'pip install couchbase==<version number>' substituting the appropriate version number.
You can see the pip command line arguments to install specific versions on the xref:project-docs:sdk-release-notes.adoc[release notes page].
====
...
It should work on Python 2.7, but is not officially supported.
...
This is fully described in the section xref:6.5@server:manage:manage-security/manage-users.nd-roles.adoc[Authorization].
...
The API reference is generated for each release and can be found linked from the xref:project-docs:sdk-release-notes.adoc[release notes for your version of the Python SDK].
