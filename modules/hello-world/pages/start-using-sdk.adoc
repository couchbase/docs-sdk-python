...
NOTE: Currently the Python Client requires the OpenSSL headers and libraries that the Python client itself was built
against to be installed prior to the client itself. Better packaging for OpenSSL support is currently being developed.
...
We are working on providing OpenSSL support for Windows shortly - it is possible
to build and run against an OpenSSL compiled for Windows manually given the
correct version of the OpenSSL headers and libraries are visible to the compiler, and
the OpenSSL dynamic libraries are in the PATH at runtime.
