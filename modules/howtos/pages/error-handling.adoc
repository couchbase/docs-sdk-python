...
Consider decorator that provides flexibility to determine which Exceptions to retry and how to retry (fixed, linear or exponential delay).
...
So `TimeoutException` is one ambiguous exception, another is `DurabilitySyncWriteAmbiguousException`, which can returned when performing a durable operation.
