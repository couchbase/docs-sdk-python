...
[source,python]
----
include::devguide:example$python/managing_connections.py[tag=simpleconnect,indent=0]
----
...
Metadata from the Python SDK’s C++ core provides information about where OpenSSL’s default certificate store is located, which version of the Mozilla CA certificate store was bundled, and other useful details.
