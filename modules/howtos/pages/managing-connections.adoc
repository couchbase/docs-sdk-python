...
If the detection heuristic fails in your environment, you can override it by setting the `ClusterOptions` `network` option (or in the connection string) to `default` if the client and server are on the same network, or `external` if they're on different networks.
