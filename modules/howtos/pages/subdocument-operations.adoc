...
Couchbase's xref:2.7@dotnet-sdk::durability.adoc[traditional durability], using `PersistTo` and `ReplicateTo`, is still available, particuarly for talking to Couchbase Server 6.0 and earlier:

[source,python]
----
include::example$subdocument_ops.py[tag=old_durability]
----
