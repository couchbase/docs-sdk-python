...
`scope` and `collection` methods should be marked as "uncommitted"

* https://issues.couchbase.com/browse/PYCBC-584[PYCBC-584]:
Support fulldoc insert with subdoc upsert with LCB V4 API

* https://issues.couchbase.com/browse/PYCBC-644[PYCBC-644]:
Update C code for logging, subdocs, `LCB_STORE_**`, `lcb_*3`, `enable_mutation_tokens` refactorings

* https://issues.couchbase.com/browse/PYCBC-645[PYCBC-645]:
Handle new safety checks in LCB for bad hostname

* https://issues.couchbase.com/browse/PYCBC-652[PYCBC-652]:
Refactor for new `lcb_createopts_**` API

* https://issues.couchbase.com/browse/PYCBC-654[PYCBC-654]:
Refactor for temp removal of `observe`/`endure`/`mctx` code from LCB3
