...
Ensure the `acouchbase API` can handle iterating over non-kv result sets greater than 1000.

* https://issues.couchbase.com/browse/PYCBC-1171[PYCBC-1171]:
Update how the `txcouchbase API` handles setting and clearing callbacks for `AsyncResult` 
to avoid unbounded memory increases.
...
Update search bindings since https://issues.couchbase.com/browse/CCBC-1428[CCBC-1428] has been resolved.

* https://issues.couchbase.com/browse/PYCBC-1157[PYCBC-1157]:
Cloud Connection sample code update.

* https://issues.couchbase.com/browse/PYCBC-1173[PYCBC-1173]:
Bump the `libcouchbase` version bundled in the Python SDK to 3.2.1.
...
Provide users an API within the Python client to work with the Couchbase Server eventing API.
...
In order to align with the RFC, the search API sort functionality can now use the child classes
derived from the Sort() base class.
