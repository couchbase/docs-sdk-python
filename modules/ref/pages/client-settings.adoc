...
This profile changes the default timeouts -- which are themselves all inherited from the underlying core {cpp} SDK.
...
.Bootstrap Timeout
====
For the SDKs built upon the core {cpp} SDK, the bootstrap timeouts are picked up from the {cpp} SDK, and include the following:
...
* The DNS SRV lookup occurs before we start the bootstrap process -- while technically a part of bootstrapping (and `dns_srv_timeout`'s default value is inherited from the {cpp} SDK), the timeout is actually outside the `bootstrap_timeout`.
====
