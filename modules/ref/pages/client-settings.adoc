...
Beyond the connection string, most settings are changed by calls to libcouchbase -- refer to the https://docs.couchbase.com/sdk-api/couchbase-c-client/group__lcb-cntl.html[API doc on `lcb_cntl()`] and the https://docs.couchbase.com/sdk-api/couchbase-c-client/group__lcb-cntl-settings.html[settings list].
...
For example, `ClusterTimeoutOption` is set via `ClusterOption.__init__` 's `timeout_config` parameter, `ClusterTracingOptions` is set via the `tracing_options` parameter, and so on.
